...
const deleteCompany = async (req, res) => {
    try {
        ...
        // Also disable Recruiter login?
        if (company.recruiter) {
            await User.findByIdAndUpdate(company.recruiter, { isDeleted: true });
        }
        ...
        res.json({
            success: true,
            message: 'Company and Recruiter soft deleted'
        });
    } catch (error) {
        res.status(500).json({
            success: false,
...
const createDrive = async (req, res) => {
    try {
        ...
        await logActivity(req, 'CREATE', 'Job', job._id, { title: job.title });
...
const deleteStudent = async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            message: 'Student account soft deleted'
        });
    } catch (error) {
        res.status(500).json({
            success: false,
...
const updateDriveStatus = async (req, res) => {
    ...
    try {
        ...
        await logActivity(req, 'UPDATE', 'Job', job._id, { status: job.status });
