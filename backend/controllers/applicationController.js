...
const applyForJob = async (req, res) => {
    ...
    try {
        ...
        if (job.minCGPA > 0 && studentCGPA < job.minCGPA) {
            return res.status(400).json({
                success: false,
                message: `Minimum CGPA required: ${job.minCGPA}. Your CGPA: ${studentCGPA}. Update your profile.`
            });
        }

        // Check Eligibility (Course) - Strict Check
        if (job.eligibleCourses && job.eligibleCourses.length > 0) {
            const studentCourse = student.profile?.course;
            if (!job.eligibleCourses.includes(studentCourse)) {
                return res.status(400).json({
                    success: false,
                    message: `This job is restricted to: ${job.eligibleCourses.join(', ')}. Your course: ${studentCourse}.`
                });
            }
        }
