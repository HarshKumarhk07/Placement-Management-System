...
const registerStudent = async (req, res) => {
    const {
        ...
        course, college, year,
        skills, projects, internships, socialLinks, academics
    } = req.body;
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (user && (await user.matchPassword(password))) {
            ...
            // Auto-link Recruiter if not already linked
            if (user.role === 'recruiter' && !user.profile?.companyId) {
                try {
                    ...
                    if (company) {
                        ...
                        user.profile = { ...user.profile, companyId: company._id };

                        // Update in DB
                        await User.updateOne({ _id: user._id }, { 'profile.companyId': company._id });

                        if (!company.recruiter) {
                            company.recruiter = user._id;
                            await company.save();
                        }
                        console.log(`🔗 Auto-linked Recruiter ${user.email} to Company ${company.name}`);
                    }
                } catch (err) {
            ...
            res.json({
                ...
                data: {
                    ...
                    email: user.email,
                    role: user.role,
                    // Ensure profile is sent with updated companyId
